...
public class ActualBreathController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //Debug.Log("loudnessFactor: " + loudnessFactor + " readyToCount: " + readyToCount + " currentScale: " + currentScale);
        if (loudnessFactor > blowThreshold)
        {
            ...
            if (readyToCount)
            {
                readyToCount = false;
                //breathCount++;
                actualManager.UpdateLog(ActualManager.Mode.Breath, 100f);
        ...
        {
            if (currentScale <= defaultYScale + 0.05f)
            {
                readyToCount = true;
            }
