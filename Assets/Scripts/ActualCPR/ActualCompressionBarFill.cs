...
public class ActualCompressionBarFill : MonoBehaviour
{
    [Header("References")]
    ...
    void Update()
    {
        ...
        //Debug.Log("Controller Y: " + controllerY + ", normalized: " + normalized);
        ...
        else if (normalized < lastNormal - 0.001f) // tay đang đi lên
        {
            if (isGoingDown && !reachedBottom)
            {
                ...
                bpmScore = BPMScore(curBPM);
                finalScore = (bpmScore + compressScore) / 2f;
                actualManager.UpdateLog(ActualManager.Mode.Compression, finalScore);
            }
        }
        lastNormal = normalized;
    }
